...
class NetworkTest(unittest.TestCase):
    ...
    def get_random_network(self):
        ...
        return random_number, net.Network(
            [random_number*3, random_number*2, random_number*3, random_number])

    def test_correct_output_size(self):
        self.assertEqual(len(self.network_a.pass_all_layers(
            [np.random.rand()]*10)), self.network_a.network_layers[-1])
        self.assertEqual(len(self.network_b.pass_all_layers(
            [np.random.rand()]*8)), self.network_b.network_layers[-1])

        random_number, network_c = self.get_random_network()
        self.assertEqual(len(network_c.pass_all_layers(
            [np.random.rand()]*random_number*3)), random_number)
